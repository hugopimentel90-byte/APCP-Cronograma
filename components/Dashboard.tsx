...
const Dashboard: React.FC<DashboardProps> = ({ project, tasks, resources, onEditProject }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex justify-between items-start gap-4">
        ...
        <button 
          ...
          className="px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-xs font-bold text-slate-500 hover:bg-slate-100 hover:text-blue-600 transition-all flex items-center gap-1.5 shadow-sm"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Progresso Real</h3>
          <p className="text-3xl font-extrabold text-gray-900 mt-2">{avgProgress.toFixed(1)}%</p>
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Previsão Término</h3>
          <p className="text-2xl font-extrabold text-gray-900 mt-2 truncate">{completionForecast.date}</p>
          <p className={`text-[10px] mt-2 font-bold uppercase ${completionForecast.color}`}>
            {completionForecast.status}
          </p>
        </div>

        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Custo Planejado</h3>
          <p className="text-2xl font-extrabold text-gray-900 mt-2">R$ {totalPlannedCost.toLocaleString()}</p>
          <p className="text-[10px] text-gray-400 mt-2 font-bold uppercase">Baseado no plano</p>
        </div>

        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Custo Realizado</h3>
          <p className="text-2xl font-extrabold text-blue-600 mt-2">R$ {totalRealizedCost.toLocaleString()}</p>
          <p className={`text-[10px] mt-2 font-bold uppercase ${costVariance >= 0 ? 'text-green-600' : 'text-red-600'}`}>
            {costVariance >= 0 ? `Economia: R$ ${costVariance.toLocaleString()}` : `Desvio: R$ ${Math.abs(costVariance).toLocaleString()}`}
          </p>
        </div>

        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Saldo Orçamentário</h3>
          <p className="text-2xl font-extrabold text-gray-900 mt-2">R$ {budgetRemaining.toLocaleString()}</p>
          <p className="text-[10px] text-gray-400 mt-2 truncate font-bold uppercase">de R$ {project.budget.toLocaleString()}</p>
        </div>

        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Saúde Financeira</h3>
          <p className={`text-lg font-extrabold mt-2 ${totalRealizedCost > project.budget ? 'text-red-600' : 'text-green-600'}`}>
          ...
          <div className="mt-4 flex items-center gap-2">
             ...
             <span className="text-[10px] text-gray-400 font-bold uppercase tracking-tighter">Status Financeiro</span>
          </div>
        </div>
      </div>
